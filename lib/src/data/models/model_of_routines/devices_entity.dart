...
class DevicesEntity {
  ...
  final List<PropertiesEntity>? properties;
	...
	Devices mapToDomain() =>
 Devices(this.id, this.name, this.manufactureName, this.deviceVersion, this.activityState, this.properties?.map((ele) => ele.mapToDomain())
.toList());
	factory DevicesEntity.mapFromDomain(Devices? domain) =>
 DevicesEntity(domain?.id, domain?.name, domain?.manufactureName, domain?.deviceVersion, domain?.activityState, domain?.properties?.map((ele) => PropertiesEntity.mapFromDomain(ele))
.toList());
}